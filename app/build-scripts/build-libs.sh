  ...
  for MODE in debug release; do
    ...
    for dir in "$LIB_DIR"/*/; do
      ...
      if [ -f "$C_SOURCE" ]; then
        ...
        echo "[build_libs_dynamic] ⚠️ Skipped $NAME (missing ${NAME}.c)"
      fi
    done
  done
  ...
  for MODE in debug release; do
    ...
    for dir in "$LIB_DIR"/*/; do
      ...
      if [ -f "$C_SOURCE" ]; then
        ...
        zig cc -target "$ZIG_TARGET" $ZIG_FLAGS -c "$C_SOURCE" -I"$dir" -std=c17 -o "$OBJ"

        if [ "$MODE" = "release" ]; then
          bash "$SCRIPT_DIR/strip.sh" --bin "$OBJ" --target-triple "$TARGET_TRIPLE"
        fi

        zig ar rcs "$ARCHIVE" "$OBJ"
        rm -f "$OBJ"
        ...
        echo "[build_libs_static] ⚠️ Skipped $NAME (missing ${NAME}.c)"
      fi
    done
  done
